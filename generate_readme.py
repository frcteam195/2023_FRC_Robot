...
and then run `./ros_dev/mkrobot.sh build` to build all the nodes. To deploy to the ROS robot, run `./ros_dev/mkrobot.sh deploy`. The naming standard for nodes is `[nameofnode]_[year]_node`.'
if len(sys.argv) > 1:
